...
Tree::Tree(glm::vec3 pos,Game* g) : Object(pos,g)
{
  initialiseTriangles();
...
void Tree::initialiseTriangles()
{
	...
	{
		addPoint(y,
             glm::vec3(0.5f*(float)sin(y/5.f*3.1415),
						 -1.f,
						 0.5f*(float)cos(y/5.f*3.1415)),
             glm::vec3((float)sin(y/5.f*3.1415),0,(float)cos(y/5.f*3.1415)),
             0.58f,0.35f,0.09f);
		editTextureCoord(y,y*0.0345f,0);
	}
	addPoint(11,glm::vec3(0.f,10.f,0.f),glm::vec3(0,1,0),0.58f,0.35f,0.09f);
	...
	for (int i = 0; i<4;i++)
	{
		...
		{
			addPoint(12+i*12+y,
               glm::vec3((4-i)*(float)sin(y/5.f*3.1415),
							 (i+3-0.5f)*10.f/7,
							 (4-i)*(float)cos(y/5.f*3.1415)),
               glm::normalize(glm::vec3((float)sin(y/5.f*3.1415),0.7f,(float)cos(y/5.f*3.1415)))
               ,0.25f,0.5f,0.15f);
			editTextureCoord(12+i*12+y,(float)(0.77+0.23*sin(y/5.f*3.1415)),(float)(0.5+0.23*cos(y/5.f*3.1415)));
		}
		addPoint(12+i*12+11,glm::vec3(0.f,(i+3)*10.f/7,0.f),glm::vec3(0,1,0),0.25f,0.5f,0.15f);
